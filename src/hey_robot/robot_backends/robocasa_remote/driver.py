...
class RoboCasaRemoteDriver:
    ...
    async def capabilities(self) -> RobotCapabilities:
        return RobotCapabilities(
            ...
            control_hz=float(self.context.settings.get("control_hz", 20.0)),
...
def create_driver(context: RobotDriverContext) -> RoboCasaRemoteDriver:
    """Build the remote driver without exposing transport construction to Manager."""
    from hey_robot.robot_backends.robocasa_remote.client import (
        GrpcRoboCasaRuntimeClient,
    )

    settings = context.settings
    target = str(settings.get("target", "grpc://127.0.0.1:9092"))
    timeout = float(settings.get("timeout_sec", 10.0))
    return RoboCasaRemoteDriver(
        context,
        GrpcRoboCasaRuntimeClient(target, timeout_sec=timeout, role="data"),
        control_client=GrpcRoboCasaRuntimeClient(
            target, timeout_sec=timeout, role="evaluator"
        ),
    )
