...
from hey_robot.robot_backends.xlerobot.executor import XLeRobotSkillExecutor
from hey_robot.robot_backends.xlerobot.hardware.config import (
    hardware_config_from_settings,
)
from hey_robot.robot_backends.xlerobot.hardware.native import NativeXLeRobotClient
...
class XLeRobotDriver:
    ...
    def __init__(self, context: RobotDriverContext) -> None:
        ...
        self.client: XLeRobotClientProtocol = NativeXLeRobotClient(
    ...
    async def start(self) -> None:
        ...
        self.startup_diagnostics = await asyncio.to_thread(
            ...
            video_timeout_ms=int(
                self.context.settings.get("startup_video_timeout_ms", 3000)
            ),
        )
    ...
    async def capabilities(self) -> RobotCapabilities:
        return RobotCapabilities(
            ...
            control_hz=float(self.context.settings.get("control_hz", 2.0)),
            ...
            metadata={
                ...
                "cameras": list(self.hardware_config.cameras.keys()),
                "safety": dict(self.context.settings.get("safety", {}) or {}),
            },
        )
    ...
    async def health(self) -> RobotHealth:
        return RobotHealth(
            ...
            metrics={
                ...
                **dict(self.context.settings.get("safety", {}) or {}),
            },
        )

    async def observe(self) -> DriverObservation:
        timeout_ms = int(self.context.settings.get("video_timeout_ms", 500))
