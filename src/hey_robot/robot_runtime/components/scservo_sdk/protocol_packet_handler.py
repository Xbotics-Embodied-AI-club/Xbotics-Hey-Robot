...
class protocol_packet_handler:
    ...
    def txRxPacket(self, txpacket):
        ...
        if txpacket[PKT_INSTRUCTION] == INST_READ:
            ...
            self.portHandler.setPacketTimeout(
                6
    ...
    def read2ByteRx(self, scs_id):
        ...
        data_read = (
            self.scs_makeword(data[0], data[1]) if (result == COMM_SUCCESS) else 0
        )
        return data_read, result, error
    ...
    def read4ByteRx(self, scs_id):
        ...
        data_read = (
            self.scs_makedword(
                self.scs_makeword(data[0], data[1]), self.scs_makeword(data[2], data[3])
            )
            if (result == COMM_SUCCESS)
            else 0
        )
