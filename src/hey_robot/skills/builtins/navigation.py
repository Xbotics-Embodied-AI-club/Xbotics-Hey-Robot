...
async def vln_not_migrated(ctx: SkillContext, arguments: dict[str, Any]) -> SkillResult:
    del ctx, arguments
    return SkillResult(
        False,
        "VLN native handler is not migrated yet; use the legacy bridge.",
        "failed",
        failure_mode="not_migrated",
        error="native VLN handler is not available",
    )
