...
from hey_robot.cognition.runtime.tool_executor import ToolExecutor
...
def test_tool_executor_tool_policy_blocks_before_permission(tmp_path: Path):
    registry = ToolRegistry()
    registry.register_simple(
        "move", lambda objective: f"moved {objective}", safety_level="actuate"
    )
    ...
    executor = ToolExecutor(
        ...
        tool_policy_resolver=ToolPolicyResolver(
            registry,
            policy=ToolPolicy(deny_safety_levels=("actuate",)),
        ),
    )
    ...
    assert result.success is False
    assert result.tool_policy_behavior == "deny"
    assert result.tool_policy_rule == "deny_safety_levels"
    records = [
        json.loads(line)
        for line in (tmp_path / "agent_runs" / "s1" / "tool_calls.jsonl")
        .read_text(encoding="utf-8")
    ...
    assert records[0]["tool_policy_behavior"] == "deny"
    assert records[0]["tool_policy_rule"] == "deny_safety_levels"
