...
from hey_robot.robot_backends.xlerobot.driver import XLeRobotDriver
from hey_robot.robot_backends.xlerobot.executor import XLeRobotSkillExecutor
from hey_robot.robot_backends.xlerobot.hardware.native import _service_diagnostic
from hey_robot.robot_hardware.mobile_base import LeKiwiBase
...
async def test_xlerobot_status_exposes_multi_arm_and_multi_camera_shape() -> None:
    config = DeploymentConfig.from_dict({"robots": {"xlerobot": {"type": "xlerobot"}}})
    driver = XLeRobotDriver(
        create_driver_context("xlerobot", config.robots["xlerobot"], "test")
    )
    driver.state = "idle"
