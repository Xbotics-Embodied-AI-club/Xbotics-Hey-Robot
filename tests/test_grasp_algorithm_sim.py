...
_tests_dir = _os.path.normpath(_this_dir)
for _i, _p in enumerate(sys.path):
    if _os.path.normpath(_p) == _tests_dir:
        sys.path.pop(_i)
        break
...
SCENE_PATH = PROJECT_ROOT / "assets" / "robots" / "xlerobot" / "dock_scene.xml"
...
def get_camera_geometry(model, data, cam_name):
    """Return (K, pos, R) for the named MuJoCo camera."""
    import mujoco
...
def project_3d_to_2d(world_point, K, cam_pos, cam_rot):
...
def validate(plane_z: float | None = None):
    ...
    print("=" * 60)
    print("GRASP ALGORITHM — SIMULATION VALIDATION")
    print("=" * 60)
    ...
    # --- 2. Camera geometry --------------------------------------------------
    print("\n[2/5] Camera geometry ...")
    cam_K, cam_pos, cam_R = get_camera_geometry(model, data, "front")
    print("  front camera (FYI):")
    print(f"    fovy   = {model.cam_fovy[0]:.1f} deg")
    print(f"    pos    = [{cam_pos[0]:.3f}, {cam_pos[1]:.3f}, {cam_pos[2]:.3f}]")
    print(f"    view   = [{cam_R[0, 2]:.3f}, {cam_R[1, 2]:.3f}, {cam_R[2, 2]:.3f}]")
    wrist_K, wrist_pos, wrist_R = get_camera_geometry(model, data, "right_wrist")
    print("  right_wrist camera (FYI):")
    print(f"    fovy   = {model.cam_fovy[1]:.1f} deg")
    print(f"    pos    = [{wrist_pos[0]:.3f}, {wrist_pos[1]:.3f}, {wrist_pos[2]:.3f}]")
    ...
    cam_K, cam_pos, cam_R = synthetic_camera_look_at(
        eye,
        wand_mid,
        fovy_deg=75.0,
    )
    print("\n  synth camera (used for validation):")
    print(f"    eye    = [{cam_pos[0]:.3f}, {cam_pos[1]:.3f}, {cam_pos[2]:.3f}]")
    print(f"    target = [{wand_mid[0]:.3f}, {wand_mid[1]:.3f}, {wand_mid[2]:.3f}]")
    print("    fovy   = 75.0 deg")
    ...
    on_plane_gt = np.array([wand_body[0], wand_body[1], effective_z], dtype=np.float64)

    all_errors_mm = []

    # --- 5a. Primary camera, wand_body (on-plane) ---
    pixel = project_3d_to_2d(wand_body, cam_K, cam_pos, cam_R)
    ...
    pixel2 = project_3d_to_2d(synth_pt, cam_K, cam_pos, cam_R)
    ...
    off_K, off_pos, off_R = synthetic_camera_look_at(
        off_eye,
        wand_body,
        fovy_deg=70.0,
    )
    pixel3 = project_3d_to_2d(wand_body, off_K, off_pos, off_R)
    ...
    pixel_g = project_3d_to_2d(wand_grasp, cam_K, cam_pos, cam_R)
    ...
    if not all_errors_mm:
        print("RESULT: FAIL — no valid test points")
        return False
    ...
    print(f"  Threshold:  {PASS_THRESHOLD_M * 1000:.0f} mm")
    ...
    if cluster_point is not None:
        ...
        print(f"  Error vs wand_body: {cluster_err:.1f} mm")
        print(f"  {'PASS' if cluster_err <= PASS_THRESHOLD_M * 1000 else 'FAIL'}")
    else:
        print("  Density-cluster sampling failed (not enough valid samples)")
