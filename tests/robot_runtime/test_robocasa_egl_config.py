...
from hey_robot.robot_runtime.robocasa_remote.egl_config import configure_headless_egl
...
def test_egl_configuration_selects_first_working_vendor(tmp_path, monkeypatch) -> None:
    ...
    monkeypatch.setattr(
        "hey_robot.robot_runtime.robocasa_remote.egl_config._NVIDIA_VENDOR", vendor
    )
    monkeypatch.setattr(
        "hey_robot.robot_runtime.robocasa_remote.egl_config._MESA_VENDOR",
    ...
    monkeypatch.setattr(
        "hey_robot.robot_runtime.robocasa_remote.egl_config.subprocess.run",
        probe_succeeds,
    )
...
def test_egl_configuration_fails_when_no_vendor_exists(tmp_path, monkeypatch) -> None:
    monkeypatch.delenv("MUJOCO_EGL_DEVICE_ID", raising=False)
    monkeypatch.setattr(
        "hey_robot.robot_runtime.robocasa_remote.egl_config._NVIDIA_VENDOR",
